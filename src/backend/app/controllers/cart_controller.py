...
from app.services.crud_service import CRUDService, ReadService, get_user_obj_by_token
...
async def create_book_cart_endpoint(access_token: str, book_id: UUID, db: AsyncSession = Depends(get_db)):
    ...
    if book_in_cart:
        ...
        return await cart_service.update({'id':cart_id, 'book_id':book_id}, cart_update, db)
...
async def update_cart_endpoint(access_token: str, book_id: UUID, amount: int, db: AsyncSession = Depends(get_db)):
    cart_id = await get_cart_id_by_token(access_token, db)
    cart = await cart_service.get_by_condition([{'id':cart_id}], db)
    if not cart:
        raise HTTPException(status_code=404, detail="Can't find cart to show")
...
async def delete_book_endpoint(access_token: str, book_id: UUID, db: AsyncSession = Depends(get_db)):
    cart_id = await get_cart_id_by_token(access_token, db)
