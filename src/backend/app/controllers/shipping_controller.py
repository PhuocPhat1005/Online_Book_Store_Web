...
async def update_shipping_endpoint(shipping_id: UUID, shipping: ShippingUpdate, db: AsyncSession = Depends(get_db)):
    return await shipping_service.update({'id': shipping_id}, shipping, db)

@router.delete("/delete_shipping", summary="Delete a shipping")
async def delete_shipping_endpoint(shipping_id: UUID, db: AsyncSession = Depends(get_db)):
