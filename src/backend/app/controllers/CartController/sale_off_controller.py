...
from app.schemas.CartSchemas.sale_off import SaleOffCreate, SaleOffUpdate
...
async def create_sale_off_endpoint(
    sale_off: SaleOffCreate, db: AsyncSession = Depends(get_db)
):
...
async def update_sale_off_endpoint(
    sale_off_id: UUID, sale_off: SaleOffUpdate, db: AsyncSession = Depends(get_db)
):
    return await sale_off_service.update({"id": sale_off_id}, sale_off, db)


@router.delete("/delete_sale_off", summary="Delete a sale off")
async def delete_sale_off_endpoint(
    sale_off_id: UUID, db: AsyncSession = Depends(get_db)
):
