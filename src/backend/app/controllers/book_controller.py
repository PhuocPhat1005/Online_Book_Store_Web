...
from app.services.crud_service import CRUDService, query_string_to_dict, query_in_db_by_id, CreateService, ReadService, UpdateService
...
async def create_author_db(file: UploadFile = File(...), db: AsyncSession = Depends(get_db)):
...
async def create_translator_db(file: UploadFile = File(...), db: AsyncSession = Depends(get_db)):
...
@router.post("/create_publishing_company_db", summary="Create publishing company database")
async def create_publishing_company_db(file: UploadFile = File(...), db: AsyncSession = Depends(get_db)):
    create_publishing_company_service = CreateService[PublishingCompany, PublishingCompanyCreate](PublishingCompany)
    # Check if the file type is supported
    if file.content_type not in [
        'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
        'application/vnd.ms-excel'
    ...
    for _, row in excel_data.iterrows():
        ...
        if new_publishing_company.id == 'empty_uuid':
            ...
            await create_publishing_company_service.create(new_publishing_company, db, 0)
...
async def create_category_db(file: UploadFile = File(...), db: AsyncSession = Depends(get_db)):
...
async def create_book_db(file: UploadFile = File(...), db: AsyncSession = Depends(get_db)):
    ...
    # Expected columns in the Excel file
    required_columns = ['ID', 'Book_name', 'isbn', 'Price', 'Book_description', 'Book_ava', 'Language', 'Book_size', 'Page_number', 'Book_cover_type', 'Category', 'Publishing_company', 'Sale_off_name', 'Publish_date', 'Authors', 'Translators']
    ...
    for _, row in excel_data.iterrows():
        category_id = await read_category_service.get_by_condition([{'category_name': handle_nan(row['Category'],'')}], db)
        publishing_company_id = await read_publishing_company_service.get_by_condition([{'publishing_company_name': handle_nan(row['Publishing_company'],'')}], db)
        sale_off_id = await read_sale_off_service.get_by_condition([{'sale_off_name': handle_nan(row['Sale_off_name'],'')}], db)
        ...
        new_book = BookCreate(
            ...
            publishing_date = handle_date(row.get('Publish_date'), None)  # Example datetime field
        )
        book_id = new_book.id
        ...
        for author in authors:
            author_id = await read_author_service.get_by_condition([{},{'full_name': author, 'pen_name': author}], db)
            if author_id:
                book_author = Book_Author_Create(
                    book_id = book_id,
                    author_id = str(author_id[0].id)
                )
                try:
        ...
        for translator in translators:
            translator_id = await read_transalator_service.get_by_condition([{},{'full_name': translator, 'pen_name': translator}], db)
            if translator_id:
                book_translator = Book_Translator_Create(
                    book_id = book_id,
                    translator_id = str(translator_id[0].id)
                )
                try:
...
async def get_book_excel_file(db: AsyncSession = Depends(get_db)):
    ...
    book_data = []
    for book in books:
        category_name = await read_category_service.get_by_condition([{'id': book.category_id}], db)
        publishing_company_name = await read_publishing_company_service.get_by_condition([{'id': book.publishing_company_id}], db)
        Sale_off_name = await read_sale_off_service.get_by_condition([{'id': book.sale_off}], db)
        ...
        author_ids = await read_book_author_service.get_by_condition([{'book_id': book.id}], db)
        author_names = []
        for author_id in author_ids:
            author_name = await read_author_service.get_by_condition([{'id': author_id.author_id}], db)
            if author_name:
                author_names.append(author_name[0].full_name)
        author_names = ', '.join(author_names)
        
        translator_ids = await read_book_translator_service.get_by_condition([{'book_id': book.id}], db)
        translator_names = []
        for translator_id in translator_ids:
            translator_name = await read_translator_service.get_by_condition([{'id': translator_id.translator_id}], db)
            if translator_name:
    ...
    return FileResponse(excel_file, filename="books.xlsx", media_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
...
async def get_author_excel_file(db: AsyncSession = Depends(get_db)):
    ...
    excel_file = create_excel_file(df, excel_file)
    return FileResponse(excel_file, filename="authors.xlsx", media_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
...
async def get_translator_excel_file(db: AsyncSession = Depends(get_db)):
    ...
    excel_file = create_excel_file(df, excel_file)
    return FileResponse(excel_file, filename="translators.xlsx", media_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")

@router.get("/get_publishing_company_excel_file", summary="Download publishing company excel file")
async def get_publishing_company_excel_file(db: AsyncSession = Depends(get_db)):
    read_publishing_company_service = ReadService[PublishingCompany](PublishingCompany)
    publishing_companies = await read_publishing_company_service.get_by_condition([{'id': ''}], db, 0)
    ...
    excel_file = create_excel_file(df, excel_file)
    return FileResponse(excel_file, filename="publishing_companies.xlsx", media_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
...
async def get_category_excel_file(db: AsyncSession = Depends(get_db)):
    ...
    excel_file = create_excel_file(df, excel_file)
    return FileResponse(excel_file, filename="categories.xlsx", media_type="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
        ...
        for author in book_author:
            ...
            if db_objs:
                if db_objs[0].full_name not in unique_list:
                    ...
                    author_data.append({"Full_name": db_objs[0].full_name, "Pen_name": db_objs[0].pen_name})
    return author_data
    ...
    if not book_translator:
        ...
        for translator in book_translator:
            ...
            if db_objs:
                if db_objs[0].full_name not in unique_list:
                    unique_list.append(db_objs[0].full_name)
                    translator_data.append({"Full_name": db_objs[0].full_name, "Pen_name": db_objs[0].pen_name})
