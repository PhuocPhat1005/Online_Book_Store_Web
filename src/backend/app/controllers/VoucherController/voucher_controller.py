...
async def create_voucher_endpoint(voucher: VoucherCreate, db: AsyncSession = Depends(get_db)):
...
async def update_voucher_endpoint(voucher_id: str, voucher: VoucherUpdate, db: AsyncSession = Depends(get_db)):
